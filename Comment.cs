...
namespace DAL
{
    public class Comment
    {
        ...
        public IEnumerable<Model.CommentNo> GetPage(Model.Page page)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "with a as(select row_number() over(order by " +
                    "commentTime desc) as num, Comment.*,workName from Comment " +
                    "join workinfo on Comment.workId=workinfo.workId)";
